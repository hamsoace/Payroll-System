...
const DetailRow: React.FC<{ label: string; value: string | number; className?: string }> = ({ label, value, className = '' }) => (
    <div className="flex justify-between py-2 border-b border-gray-100">
        ...
        <span className={`text-sm font-semibold text-gray-800 ${className}`}>{typeof value === 'number' ? `$${value.toFixed(2)}` : value}</span>
    </div>
);
        ...
        <div className="mt-6 border-t-2 border-gray-200 pt-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-green-50 p-4 rounded-md text-center">
                    ...
                    <p className="text-xl font-bold text-green-700">${payroll.grossPay.toFixed(2)}</p>
                </div>
                <div className="bg-red-50 p-4 rounded-md text-center">
                    <p className="text-sm font-medium text-red-800">Total Deductions</p>
                    <p className="text-xl font-bold text-red-700">${payroll.totalDeductions.toFixed(2)}</p>
                </div>
                <div className="bg-blue-100 p-4 rounded-md text-center border-2 border-blue-600">
                    ...
                    <p className="text-3xl font-extrabold text-blue-800">${payroll.netPay.toFixed(2)}</p>
                </div>
            </div>
        </div>
